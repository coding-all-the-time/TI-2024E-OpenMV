...
def detect_individual_cells(img, BOARD_ROI):
    ...
    # 计算每个块到中心的距离和角度
    def get_angle_distance(cx, cy):
        dx = cx - avg_x
        dy = avg_y - cy  # 图像Y轴向下为正
        distance = math.sqrt(dx**2 + dy**2)
        angle = math.atan2(dy, dx)  # -π到π
        return angle, distance

    # 分类存储
    center_blob = None
    inner_circle = []  # 1,3,7,9
    outer_circle = []  # 2,4,6,8

    for cx, cy, b in centers:
        angle, dist = get_angle_distance(cx, cy)

        # 自动分类（根据距离比例）
        if dist < min(b.w(), b.h()) * 0.7:  # 中心块
            center_blob = b
        else:
            # 临时存储需要后续分类
            if not inner_circle or dist > inner_circle[0][1]:
                # 更远的放入内圈
                outer_circle.extend(inner_circle)
                inner_circle = [(angle, dist, cx, cy, b)]
            else:
                outer_circle.append((angle, dist, cx, cy, b))

    # 确保分类正确（如果自动分类不理想可以改用固定阈值）
    if len(inner_circle) < 4:
        # 按距离重新分类：取距离最远的4个作为内圈
        all_outer = inner_circle + outer_circle
        all_outer.sort(key=lambda x: -x[1])
        inner_circle = all_outer[:4]
        outer_circle = all_outer[4:]

    # 角度分区函数
    def get_quadrant(angle):
        """将角度映射到0-3的象限编号"""
        angle = (angle + math.pi) % (2*math.pi)  # 转换为0-2π
        return int(angle // (math.pi/2)) % 4

    # 内圈编号（1,3,9,7）
    inner_numbers = {0:7, 1:9, 2:3, 3:1}  # 根据实际调整
    inner_cells = {}
    for angle, dist, cx, cy, b in inner_circle:
        quad = get_quadrant(angle)
        inner_cells[inner_numbers[quad]] = b

    # 外圈编号（2,6,8,4）
    outer_numbers = {0:8, 1:6, 2:2, 3:4}  # 根据实际调整
    outer_cells = {}
    for angle, dist, cx, cy, b in outer_circle:
        quad = get_quadrant(angle - math.pi/4)  # 偏移45度
        outer_cells[outer_numbers[quad]] = b

    # 合并所有编号
    numbered_cells = {5: center_blob}
    numbered_cells.update(inner_cells)
    numbered_cells.update(outer_cells)

    # 创建ROI矩阵
    rois = [[None]*3 for _ in range(3)]
    # 编号到矩阵位置的映射
    num_to_pos = {
        1: (0, 0), 2: (0, 1), 3: (0, 2),
        4: (1, 0), 5: (1, 1), 6: (1, 2),
    ...
    for num in numbered_cells:
        if num not in num_to_pos:
            continue
        b = numbered_cells[num]
        ...
        rois[row][col] = (x, y, w, h)
        img.draw_rectangle(x, y, w, h, color=(255, 0, 0))
        img.draw_string(x+5, y+5, str(num), color=(255, 0, 0))
